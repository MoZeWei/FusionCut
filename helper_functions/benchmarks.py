...
from qcg.generators import gen_supremacy, gen_hwea, gen_BV, gen_sycamore, gen_adder, construct_custom
...
#gates: [(a,b)...], it already follows the tpologic order
def construct_micro(nqubit, reg_name, gates):
    circ = qiskit.QuantumCircuit(qiskit.QuantumRegister(nqubit, reg_name))

    circ.h(range(nqubit))

    for gate in gates:
        control_bit, target_bit = gate
        circ.cnot(control_bit, target_bit)
    
    return circ

def generate_gates(nqubit, ngate, mode):
    gates = list()
    if mode == "simple":
        #(1,8,1,2,3)
        if ngate != 15 or nqubit != 10:
            print("Number of gate or qubit is wrong")
            exit(1)
        gates.append((0,1))
        for target in range(2,nqubit):
            gates.append((1,target))
...
def generate_circ(num_qubits, depth, circuit_type, reg_name, connected_only, seed):
    ...
    while num_trials:
        if circuit_type == "supremacy":
            ...
            density += 0.001
        elif circuit_type == "MICRO":
            gates = generate_gates(num_qubits, 15, "simple")
            full_circ = construct_micro(num_qubits, reg_name, gates)
            #print the circuit 
            # print(full_circ)
        elif circuit_type == "random":
            full_circ = construct_random(num_qubits=num_qubits, depth=depth)
        elif circuit_type == "mycirc":
            full_circ = construct_custom()
        else:
