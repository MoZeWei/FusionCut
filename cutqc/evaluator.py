    ...
    for subcircuit_idx in subcircuit_instances:
        ...
        for rank in range(num_workers):
            ...
            if len(rank_jobs) > 0:
                ...
                python_command = (
                    "python3 -m cutqc.parallel_run_subcircuits --data_folder %s --subcircuit_idx %d --rank %d"
...
def measure_prob(unmeasured_prob, meas):
    ...
    if meas.count("comp") == len(meas) or type(unmeasured_prob) is float:
        ...
        #print("Measured Probability from evaluator file in measure prob function",measured_prob)
    ...
    for subcircuit_idx in subcircuit_entries:
        ...
        for rank in range(num_workers):
            ...
            if len(rank_jobs) > 0:
                ...
                python_command = (
                    "python3 -m cutqc.parallel_attribute_shots --data_folder %s --subcircuit_idx %d --rank %d"
                    % (data_folder, subcircuit_idx, rank)
                )
                proc = subprocess.Popen(python_command.split(" "))
                procs.append(proc)
