...
class MIP_Model(object):
    def __init__(
        ...
        n_vertices,
        edges,
        vertex_ids,         ##This is the node_name_ids in read_circ {vectex_name: linearid}
        id_vertices,        ##This is the id_node_names in read_circ {linearid: vertex_name}
        num_subcircuit,
        ...
        self.gatename_fusion_weights = gatename_fusion_weights
    ...
    def _add_variables(self):
        ...
        ## Total fusion weights of original circuit
        total_fusion_weights = 0
        for gateid, weight in self.gatename_fusion_weights.items():
            total_fusion_weights += weight

        self.total_fusion_weights = total_fusion_weights
        ...
        for subcircuit in range(self.num_subcircuit):
            ...
            self.subcircuit_counter[subcircuit]["fusion_weight"] = self.model.addVar(
                lb = 1,
                ub = self.total_fusion_weights, ##QUES: Should we narrow down the range for a good answer?
                vtype=gp.GRB.INTEGER,
                name="fusion_weight_%d" % subcircuit
            )
            self.subcircuit_counter[subcircuit]["log_fusion_weight"] = self.model.addVar(
                vtype=gp.GRB.CONTINUOUS, name="log_fusion_weight_%d" %subcircuit
            )
            ...
            #TODO: Is there a better way to use a variable to calculate the product(sum of gate fusion weight of each sub-circuit)
    ...
    def _add_constraints(self):
        ...
        ##The Dummy variable will always be zero.
        ##Just added it cause I dont know how to get past the none type and grb type error.
        ...
        for subcircuit in range(self.num_subcircuit):
            ...
            ## rho: initialzation qubit generated by cutting
            self.model.addConstr(
            ...
            ## O: new measurement qubit generated by cutting
            self.model.addConstr(
            ...
            ## fusion_weight: sum of fusion weight of gates in each sub-circuit
            self.model.addConstr(
                self.subcircuit_counter[subcircuit]["fusion_weight"]
                == gp.quicksum(
                    [self.vertex_var[subcircuit][v] * self.gatename_fusion_weights[self.id_vertices[v]]
                     for v in range(self.n_vertices)]       ##TOCHECK
                )
            )

            self.model.addGenConstrLog(self.subcircuit_counter[subcircuit]["fusion_weight"],
                                       self.subcircuit_counter[subcircuit]["log_fusion_weight"])
...
def read_circ(circuit):
    ...
    for vertex in dag.topological_op_nodes():
        ...
        qubit_gates_list[arg0].append(vertex)
        qubit_gates_list[arg1].append(vertex)
        ...
        if vertex_name not in node_name_ids and id(vertex) not in vertex_ids:
            ...
            id_vertex[curr_node_id] = id(vertex)
            curr_node_id += 1

    ### For gates sharing the same control bits with others, we assign them a weight,
    ### while for others do not, we assign them a weight of 1
    coef = 10
    for qubit in dag.qubits:
        for i in range(0,len(qubit_gates_list[qubit])):
            gate = qubit_gates_list[qubit][i]
            cb, tb = gate.qargs
            ### When tb is current qubit, we wont check for its sharing here
            ### We will check for such a gate when the qubit is its cqb
            if tb == qubit:
                continue
            last_cqb = -1
            last_tqb = -2
            ...
            next_tqb = -4
            if i != 0:
                last_cqb, last_tqb = qubit_gates_list[qubit][i-1].qargs
            if i != len(qubit_gates_list[qubit]) - 1:
                next_cqb, next_tqb = qubit_gates_list[qubit][i+1].qargs
            if last_cqb == qubit or next_cqb == qubit or (last_tqb == qubit and next_tqb == qubit):
                gatename_fusion_weights[id_node_names[vertex_ids[id(gate)]]] = coef
                # print(id_node_names[vertex_ids[id(gate)]])
            else:
                gatename_fusion_weights[id_node_names[vertex_ids[id(gate)]]] = 1
    ...
    n_vertices = dag.size()
    if(n_vertices != len(gatename_fusion_weights)):
        print("wrong size of gatename_fusion_weights")
        exit(1)

    return n_vertices, edges, node_name_ids, id_node_names, gatename_fusion_weights
...
def find_cuts(
    ...
    n_vertices, edges, vertex_ids, id_vertices, gatename_fusion_weights = read_circ(circuit=stripped_circ)
    ...
    for num_subcircuit in num_subcircuits:
        ...
        kwargs = dict(
            ...
            max_cuts=max_cuts,
            gatename_fusion_weights = gatename_fusion_weights,
        )
